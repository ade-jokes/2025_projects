...
async def create_todo(request: Request, session: SessionDep):
    ...
    if "application/json" in content_type:
        ...
        todo_data = TodoCreate(
            ...
            status=form.get("status", "not completed") 
        )
...
@app.post("/todos/delete/{todo_id}",response_class=HTMLResponse)
...
@app.post("/todos/{todo_id}/status", response_class=HTMLResponse)
async def update_status(todo_id: int, request: Request, status: str = Form(...), session: SessionDep = Depends(get_session)):
    todo = session.get(Todo, todo_id)
    if not todo:
        raise HTTPException(status_code=404, detail="Todo not found")

    todo.status = "completed" if status == "completed" else "not completed"
    session.add(todo)
    session.commit()
